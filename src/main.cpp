...
int main(int argc, char* argv[])
{
    ...
    for (int iteration = 0; iteration < ITERATIONS; ++ iteration) {
        ...
        //dump features
        char filename_featuni[256];
        char filename_feat[256];
        sprintf(filename_featuni, "tmp/feature_unigram_%d", iteration);
        sprintf(filename_feat, "tmp/feature_%d", iteration);
        Dump::dumpFeatures(filename_featuni, featuresUnigram, truth);
        Dump::dumpFeatures(filename_feat, features, truth);
        /*
        if (iteration == 0) {
